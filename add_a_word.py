...
class Database:
    ...
    def connect(self):
        try:
            self.connection = mysql.connector.connect(
                host=self.host, port=self.port, user=self.username, password=self.password, auth_plugin="mysql_native_password")
    ...
    def check_word(self, word):
        self.cursor.execute("""SELECT * FROM dictionary.word WHERE word=%s""", (word,))
        checked = self.cursor.fetchall()
        self.connection.commit()
        return checked

    def update_word(self, new, old):
        self.cursor.execute("""UPDATE dictionary.word SET word=%s WHERE word=%s""", (new, old))
        self.connection.commit()
...
def main():
    ...
    if answer:
        ...
        database.update_word(new_answer, word)
    else:
